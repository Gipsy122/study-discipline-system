...
onValue(statsRef, (snapshot) => {
    ...
    if (d) {
        ...
        if(isAdmin) document.getElementById('admin-panel').style.display = 'grid';
    }
});
...
onValue(timerRef, (snapshot) => {
    ...
    if (t && t.running) {
        ...
        ticker = setInterval(() => {
            ...
            clock.innerText = Math.floor(diff/60) + ":" + String(diff%60).padStart(2,'0');
        }, 1000);
    } else {
...
window.manualInput = (name, limit, isStrict) => {
    const used = prompt(`Minutes spent on ${name}:`);
